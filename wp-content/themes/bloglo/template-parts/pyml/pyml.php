...
$bloglo_posts_per_page = 'col-md-' . ceil( esc_attr( 12 / $bloglo_args['posts_per_page'] ) ) . ' col-sm-12 col-xs-12';

while ( $bloglo_posts->have_posts() ) :
	$bloglo_posts->the_post();

	// Background images HTML markup.
	// $bloglo_pyml_bgs_html .= '<div class="pyml-slide-bg" style="background-image:url(' . get_the_post_thumbnail_url( get_the_ID(), 'full' ) . ')"></div>';
